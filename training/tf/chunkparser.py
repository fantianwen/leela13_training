...
class ChunkParser:
    def __init__(self, chunkdatasrc, shuffle_size=1, sample=1,
        ...
        self.prob_reflection_table = [
            [remap_vertex(vertex, sym)
              for vertex in range(TOTAL_CROSSING)]+[TOTAL_CROSSING] for sym in range(8)]
        # Build full 16-plane reflection tables.
        self.full_reflection_table = [
            np.array([remap_vertex(vertex, sym) + p * TOTAL_CROSSING
                for p in range(16) for vertex in range(TOTAL_CROSSING)])
                    for sym in range(8)]
        ...
        self.flat_planes = [ b'\1'*TOTAL_CROSSING + b'\0'*TOTAL_CROSSING, b'\0'*TOTAL_CROSSING + b'\1'*TOTAL_CROSSING ]
    ...
    def convert_v1_to_v2(self, text_item):
        ...
        if np.any(np.isnan(probabilities)):
            ...
            return False, None
        if not(len(probabilities) == TOTAL_CROSSING+1):
            return False, None

        probs = probabilities.tobytes()
        if not(len(probs) == (TOTAL_CROSSING+1) * 4):
            return False, None
    ...
    def v2_apply_symmetry(self, symmetry, content):
        ...
        assert len(planes) == TOTAL_CROSSING*16
        ...
        assert len(probs) == TOTAL_CROSSING+1
        probs = probs.tobytes()
    ...
    def convert_v2_to_tuple(self, content):
        ...
        assert len(planes) == (18 * TOTAL_CROSSING), len(planes)
...
class ChunkParserTest(unittest.TestCase):
    def generate_fake_pos(self):
        ...
        planes = [np.random.randint(2, size=TOTAL_CROSSING).tolist()
    ...
    def test_parsing(self):
        ...
        batch = ( np.reshape(np.frombuffer(data[0], dtype=np.uint8),
                             (batch_size, 18, TOTAL_CROSSING)).tolist(),
                  np.reshape(np.frombuffer(data[1], dtype=np.float32),
                             (batch_size, TOTAL_CROSSING+1)).tolist(),
            ...
            for symmetry in range(8):
                ...
                sym_planes = [
                    [plane[remap_vertex(vertex, symmetry)]
                        for vertex in range(TOTAL_CROSSING)]
                            for plane in planes]
                sym_probs = [
                    probs[remap_vertex(vertex, symmetry)]
                        for vertex in range(TOTAL_CROSSING)] + [probs[TOTAL_CROSSING]]
