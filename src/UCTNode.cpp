...
void UCTNode::print_candidates(int color){
    ...
    for (const auto& child : get_children()) {
        auto move_policy = child.get_policy();
        if(child->get_visits()>0) {
            ...
            printf("the move %d 's winrate is %f,and policy is %f \n", move, prob, move_policy);
        }
        total_policy+=move_policy;
    }
    printf("the total policy is: %f",total_policy);
